<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@include file="includes/header.jspf" %>

        <div class="container mt-55">
            <div class="row">
                <div class="col-sm-12"><h1 class="h-home">Транзакции</h1></div>
            </div>

            <div class="row">
            <form>
                <div class="col-sm-5">
                    <div class="form-group">
                        <select class="form-control">
                            <option selected>0000 0000 0000 0000 - Name card - 1233,73 USD</option>
                            <option>0000 0000 0000 0001 - Name card - 1233,73 USD</option>
                            <option>0000 0000 0000 0002 - Name card - 1233,73 USD</option>
                            <option>0000 0000 0000 0003 - Name card - 1233,73 USD</option>
                            <option>0000 0000 0000 0004 - Name card - 1233,73 USD</option>
                        </select>
                    </div>
                </div>
                <div class="col-sm-5">
                    <div class="form-group">
                        <input type="text" class="form-control" id="exampleInputPhone" placeholder="Карта получателя">
                    </div>
                </div>
                <div class="col-sm-2 text-right">
                    <button type="submit" class="btn btn-success btn-md btn-block">Отправить</button>
                </div>
            </form>
            </div>
            <div class="row">
                <div class="col-md-12">

                    <h3 class="h-home">История транзакций</h3>
                    <div class="row bg-white-div">

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-plus">+22231,93 <small>USD</small></div>
                            </div>
                        </div>

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-minus">-22231,93 <small>USD</small></div>
                            </div>
                        </div>

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-plus">+22231,93 <small>USD</small></div>
                            </div>
                        </div>

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-minus">-22231,93 <small>USD</small></div>
                            </div>
                        </div>

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-minus">-22231,93 <small>USD</small></div>
                            </div>
                        </div>

                        <div class="col-sm-12 transaction-history">
                            <div class="row">
                                <div class="col-sm-9 transaction-history-name"><span class="transaction-history-date">11.11.2011 <small>13:59</small></span>от 0000 0000 0000 0000 (Имя Фамилия) на 0000 0000 0000 0000 (Имя Фамилия)</div>
                                <div class="col-sm-3 transaction-history-money money-minus">-22231,93 <small>USD</small></div>
                            </div>
                        </div>


                    </div>
                </div>
            </div>
        </div>

<%@include file="includes/footer.jspf" %>